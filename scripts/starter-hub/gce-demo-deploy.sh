...
# scripts/starter-hub/gce-demo-deploy.sh - One-stop deployment for the Fabric Demo Hub
...
echo "=== Fabric Hub Full Deployment: ${HUB_NAME} ==="
...
echo "Your Fabric Hub should now be available at https://${HUB_DOMAIN}"
echo ""
echo "Note: To enable agent telemetry, upload the GCP credentials key to the Hub:"
echo "  fabric secret set fabric-telemetry-gcp-credentials \\"
echo "    --type file --target '~/.fabric/telemetry-gcp-credentials.json' \\"
