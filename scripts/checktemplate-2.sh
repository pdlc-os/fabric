...
cd /Users/user/src/cli-projects/qa-fabric
...
cat > /tmp/test-template/fabric-agent.yaml << 'EOF'
harness: claude
...
fabric template sync test-phase2 \
...
fabric template push test-phase2 --from /tmp/test-template

# 5. Pull template to a new location
echo -e "\n[5] Pulling template..."
rm -rf /tmp/test-template-pulled
fabric template pull test-phase2 --to /tmp/test-template-pulled
...
echo -e "\n[7] Comparing files..."
diff /tmp/test-template/fabric-agent.yaml /tmp/test-template-pulled/fabric-agent.yaml && \
  echo "fabric-agent.yaml matches" || echo "MISMATCH!"
