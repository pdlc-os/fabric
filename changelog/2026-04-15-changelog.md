...
## 🚀 Features
* **[Claude Harness] Enhanced Authentication Methods:** Added support for OAuth token (`oauth-token`) and native credentials file (`auth-file`) authentication. This enables users of Claude Code (Pro, Max, Team, and Enterprise) to use their subscription-based credentials directly within FABRIC agents.
...
* **[Broker] Log Resolution:** Removed an erroneous `.fabric` suffix that was preventing correct log path resolution in the broker.
