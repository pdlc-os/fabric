...
## 🐛 Fixes
* **[Agent Auth] Robust Token Management:** Transitioned agent authentication from the `FABRIC_AUTH_TOKEN` environment variable to a canonical file (`~/.fabric/fabric-token`). This ensures all processes—including the CLI, lifecycle hooks, and child agents—always use the latest refreshed token, eliminating failures caused by stale credentials.
* **[Service Manager] Thread-Safe Service Supervision:** Introduced mutex guarding for internal service state in `fabrictool`. This resolves race conditions that caused inconsistent restart tracking and occasional crashes during rapid process exit/restart cycles.
