...
## ⚠️ BREAKING CHANGES
* **Global Profile Management:** Runtime `profiles` and `runtimes` are no longer supported in grove-level `settings.yaml`. These must now be managed exclusively at the global/broker level (`~/.fabric/settings.yaml`). Existing grove-specific profiles must be migrated to the global configuration.
