...
## ⚠️ BREAKING CHANGES
* **Chat App Command Split:** The `/fabric` slash command has been split into two separate commands to improve clarity and reduce command collisions.
    * `/fabric`: Now dedicated strictly to agent messaging.
    * `/fabricAdmin`: A new command that handles all administrative tasks, including space linking (`link`), registration (`register`), and agent management (`list`, `start`, `stop`, `create`, `delete`).
    * **Action Required:** Users and administrators must update their Google Chat App configurations to include the new `/fabricAdmin` command (Command ID 2) and update their workflows to use it for all management operations.
...
* **Agent Identity:** Added the `fabric whoami` command, allowing users and agents to verify their current identity and active CLI mode.
