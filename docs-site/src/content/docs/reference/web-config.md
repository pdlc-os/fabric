...
This document describes the configuration for the Fabric Web Dashboard. The web UI is served by the Go `fabric` binary with the `--enable-web` flag.
...
| :--- | :--- | :--- |
| `FABRIC_SERVER_WEB_PORT` | `8080` | The HTTP port the web UI listens on (overridden by `--web-port`). |
...
| :--- | :--- |
| `FABRIC_SERVER_AUTH_GOOGLE_CLIENTID` | Google OAuth 2.0 Client ID. |
| `FABRIC_SERVER_AUTH_GOOGLE_CLIENTSECRET` | Google OAuth 2.0 Client Secret. |
| `FABRIC_SERVER_AUTH_GITHUB_CLIENTID` | GitHub OAuth App Client ID. |
| `FABRIC_SERVER_AUTH_GITHUB_CLIENTSECRET` | GitHub OAuth App Client Secret. |
| `FABRIC_SERVER_AUTH_AUTHORIZEDDOMAINS` | Comma-separated list of email domains allowed to sign in. |
...
| Variable | Default | Description |
| :--- | :--- | :--- |
| `FABRIC_DEV_TOKEN` | | Explicit development token for Hub API access. |
| `FABRIC_DEV_TOKEN_FILE` | `~/.fabric/dev-token` | Path to the token file generated by the Hub. |
