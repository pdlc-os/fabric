...
description: Comprehensive overview of Fabric's security model, including authentication, transport security, and permissions.
---

Fabric is designed with a multi-layered security model to ensure the integrity and confidentiality of agent operations, user data, and system communications. This document outlines the authentication mechanisms, transport security protocols, and future authorization plans for the Fabric platform.

## 1. Authentication Model

Fabric operates in multiple contexts, each with specific security requirements. Authentication is managed centrally by the **Fabric Hub**, which resolves identities for users, agents, and infrastructure components.
...
| **CLI (Hub Commands)** | Terminal | OAuth 2.0 + Device Flow | `~/.fabric/credentials.json` |
| **Agent (fabrictool)** | Container | Hub-issued JWT | Env Var (`FABRIC_HUB_TOKEN`) |
| **Runtime Broker** | Compute Node | HMAC Signature | `~/.fabric/broker-credentials.json` |
| **Development** | Any | Developer Token (Bearer) | `~/.fabric/dev-token` |

### 1.2 User Authentication (OAuth 2.0)

For both Web and CLI access, Fabric relies on standard OAuth 2.0 providers (Google and GitHub).
...
### 1.3 Agent Authentication (`fabrictool`)
...
- **Transmission**: The token is injected into the container via the `FABRIC_HUB_TOKEN` environment variable and is used by `fabrictool` for all API calls.
...
- **Signing**: Every request includes headers for `X-Fabric-Broker-ID`, `X-Fabric-Timestamp`, `X-Fabric-Nonce`, and `X-Fabric-Signature`.
...
In production mode, Fabric mandates the use of TLS for all network traffic.
...
- **Browser/Web**: Since browser WebSocket APIs cannot set custom headers, Fabric uses a **Ticket-Based Authentication** system. The client requests a short-lived, single-use ticket via a POST request (authenticated by cookie) and provides it in the WebSocket query string (`?ticket=...`).

## 3. Authorization and Access Control

### 3.1 Domain Authorization

Fabric supports restricting authentication to specific email domains via the `FABRIC_AUTHORIZED_DOMAINS` configuration. This provides a first-line defense, ensuring only authorized organization members can access the Hub.

### 3.2 Permissions System (Future Plans)

A comprehensive, hierarchical RBAC (Role-Based Access Control) system is currently in the design phase. For a detailed technical specification of the policy language and agent identity claims, see the [Policy & Permissions Reference](/fabric/reference/permissions-policy/).
...
Fabric provides a typed, scope-aware secret management system. Secret values are never stored in plaintext in the Hub database. For a user-facing guide, see [Secret Management](/fabric/hosted/user/secrets/).
...
- GCP SM secret names follow the pattern: `fabric-{scope}-{sha256(scopeID)[:12]}-{name}`.
...
- **`variable`**: Written to `~/.fabric/secrets.json` inside the container.
- **`file`**: Written to a specified filesystem path (max 64 KiB).

### 4.4 User Access Tokens (UATs)

For headless environments (CI/CD, automation), Fabric supports **user access tokens (UATs)**.
...
Fabric ensures that sensitive credentials (GCP Service Accounts, API keys for LLMs) are propagated into agent containers securely.
...
- **Shadow Mounts**: Fabric uses `tmpfs` shadow mounts to definitively block agents from accessing `.fabric` configuration data or other agents' workspaces within the same project.
...
To facilitate local development, Fabric provides a **Development Authentication** mode.
- **Developer Token**: A persistent token starting with `fabric_dev_` stored in `~/.fabric/dev-token`.
