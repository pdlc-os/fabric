...
Every Fabric agent has a dedicated **Workspace**, mounted at `/workspace` inside the agent's container. This is where the agent reads code, makes changes, and runs commands.

Fabric provides flexible options for how this workspace is backed on your host machine, ranging from isolated git worktrees to direct directory mounts.

## Workspace Resolution

When you start an agent, Fabric determines its workspace based on the following precedence:

1.  **Explicit Workspace** (`--workspace` flag):
    If you provide a path via `--workspace`, Fabric mounts that directory directly. This works in both Git and non-Git environments.

2.  **Git Worktree** (Git repositories):
    If you are in a Git repository and do not provide an explicit workspace, Fabric uses [Git Worktrees](https://git-scm.com/docs/git-worktree) to give the agent its own isolated working directory and branch.

3.  **Project Root / CWD** (Non-Git environments):
    If you are not in a Git repository, Fabric mounts the project root (or current directory for global agents) directly.

---

## 1. Explicit Workspaces (`--workspace`)

You can tell Fabric exactly which directory to use as the workspace. This is useful for:
...
fabric start my-agent "fix bugs" --workspace ./my-service
```
...
When working inside a Git repository without an explicit `--workspace`, Fabric automatically manages **Git Worktrees**. This ensures that each agent has its own isolated checkout of the code, allowing them to work on different branches simultaneously without interfering with your main working directory.

### Prerequisites
- Git **2.47.0** or newer is required (for relative path support).

### Branch Resolution
Fabric determines which branch to check out in the worktree:

1.  **Explicit Branch** (`--branch`, `-b`):
    ```bash
    fabric start my-agent -b feature/login "add logging"
    ```
    - If the branch exists and has a worktree, Fabric **reuses the existing worktree** (see below).
    - If the branch exists but has no worktree, Fabric creates a new worktree for it.
    - If the branch doesn't exist, Fabric creates it (based on current HEAD) and a worktree.

2.  **Agent Name Matching**:
    If you don't specify a branch, Fabric checks if a branch named after the agent exists (e.g., `my-agent`).
    - **Match Found**: It behaves exactly as if you passed `-b my-agent`.
    - **No Match**: Fabric creates a new branch named `my-agent` and a corresponding worktree.

### Reusing Existing Worktrees
If you request a branch that is already checked out in another worktree (e.g., by another agent or manually created), Fabric detects this.
- Instead of failing or creating a conflict, Fabric **mounts the existing worktree path**.
...
- Fabric defaults to mounting the **project root** (the directory containing `.fabric`).
...
When a Fabric Hub is enabled, workspace strategy changes depending on the project type. The Hub supports three types of remote workspaces:
...
- The Hub automatically initializes a seeded `.fabric` structure.
...
fabric hub project create https://github.com/org/repo.git
```

#### How Git Projects Work

1. The Hub stores the git remote URL and default branch as project metadata.
2. When an agent starts, the Runtime Broker injects `FABRIC_GIT_CLONE_URL`, `FABRIC_GIT_BRANCH`, and `FABRIC_GIT_DEPTH` as environment variables.
3. The `fabrictool init` process inside the container uses a `git init` + `git fetch` strategy to provision the workspace into `/workspace`. This approach handles workspaces that may already contain `.fabric` metadata or `.fabric-volumes` directories, and properly clears stale artifacts before initialization.
4. A feature branch `fabric/<agent-name>` is created and checked out automatically.
...
Each agent gets its own branch named `fabric/<agent-name>`. This prevents conflicts when multiple agents work on the same repository concurrently.
...
|----------|-------------|---------|
| `FABRIC_GIT_CLONE_URL` | HTTPS URL of the repository to clone | *(required)* |
| `FABRIC_GIT_BRANCH` | Branch to clone | `main` |
| `FABRIC_GIT_DEPTH` | Clone depth | `1` |
...
When you link an existing local git project to a Hub (`fabric hub link`), the project becomes **Hub-managed**. Once linked, **all agents started via the Hub use clone-based provisioning**, even if the broker machine already has the repository checked out locally.
...
fabric hub disable
# or run with --no-hub
fabric start my-agent --no-hub "fix the bug"
```
...
You can manage shared directories using the `fabric shared-dir` CLI commands:

```bash
# Create a new shared directory
fabric shared-dir create <name>

# List shared directories in the current project
fabric shared-dir list

# View details about a specific shared directory
fabric shared-dir info <name>

# Remove a shared directory (permanently deletes contents)
fabric shared-dir remove <name>
```
...
- **Standard Path:** `/fabric-volumes/<name>`
- **Workspace Path:** `/workspace/.fabric-volumes/<name>`
...
Fabric provides a helper command, `cdw` (Change Directory to Worktree), to quickly navigate to an agent's workspace on your host.

```bash
fabric cdw <agent-name>
```
...
Will also take a branch/worktree name outside of fabric agents, most useful for getting back to main.
...
- **Explicit Workspaces**: Directories mounted via `--workspace` are **NOT** deleted. Fabric only cleans up resources it created.
