...
**HA hosted** is the durable, always-on [hosted](/fabric/choosing-a-mode/) availability tier. Its
...
- **Horizontal scale.** Stateless [proxy](/fabric/glossary/) and hosted brokers can be replicated
  alongside Hub instances without broker-specific scheduling.
- **Single- or multi-user.** Like every hosted tier, [tenancy](/fabric/choosing-a-mode/#tenancy)
  is orthogonal: an HA Hub can serve one user or many, with OAuth, Groups, and access policies for
  multi-user deployments.

The `postgres` database driver (`FABRIC_SERVER_DATABASE_DRIVER=postgres`) is what enables
...
HA hosted is more to run and pay for than [Single-node](/fabric/hosted/single-node/overview/). You
...
lowest cost, use [Single-node hosted](/fabric/hosted/single-node/overview/) instead.

## How it differs from Single-node hosted

| | HA hosted | [Single-node hosted](/fabric/hosted/single-node/overview/) |
|--|--|--|
...
[Choosing a Mode](/fabric/choosing-a-mode/) for the full picture.

## Next steps

The HA admin surface is the fullest of any mode:

- [Runtime Brokers & Profiles](/fabric/hosted/ha/runtime-broker/) — proxy/hosted brokers and profiles.
- [Managed Agents](/fabric/hosted/single-node/managed-agents/) — cloud-managed agents that bypass brokers.
- [Kubernetes Runtime](/fabric/hosted/ha/kubernetes/) — running agents on Kubernetes.
- [Identity & Access (RBAC)](/fabric/hosted/ha/permissions/) and [Proxy Auth (IAP)](/fabric/hosted/ha/auth-proxy-iap/).
- [Lifecycle Hooks](/fabric/hosted/ha/lifecycle-hooks/) and [Multi-Broker Setup](/fabric/hosted/ha/multi-broker/).
- [Connecting to a Hub](/fabric/hosted/user/hosted-user/) — the shared user-facing journey.
