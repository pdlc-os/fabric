...
description: Designing access control for Fabric projects and agents.
---

Fabric is implementing a robust, principal-based access control system to manage resources across distributed projects and teams. While currently in the design and early implementation phase, this document outlines the core concepts and policy model.

For a detailed technical specification of the policy language and agent identity claims, see the [Policy & Permissions Reference](/fabric/reference/permissions-policy/).
...
Fabric enforces strict policy-based authorization for all agent operations:
...
Fabric uses a **Hierarchical Override Model** for policies. Policies can be attached at three levels:
...
### Resolution Logic
When an action is attempted, Fabric resolves effective permissions by traversing the hierarchy from the most specific to the most general:
...
To simplify management, Fabric provides built-in roles that bundle common permissions:
...
Fabric enforces a robust security model for agent-to-agent interactions (progeny) through **Ancestry Chains** and **Transitive Access Control**.
...
The Fabric Web Dashboard includes a centralized **Admin Management Suite** (accessible to users with administrative privileges) that provides dedicated views for access control management:
...
By leveraging these administrative views, Platform Ops can efficiently map their organization's structure directly into Fabric's Principal and Policy hierarchy.