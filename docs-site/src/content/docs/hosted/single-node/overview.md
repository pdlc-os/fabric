...
**Single-node hosted** is the cheaper, simpler of the two [hosted](/fabric/choosing-a-mode/)
...
follow the [Hub Setup](/fabric/hosted/single-node/hub-server/) guide.

## What this tier provides

- **A networked Hub.** Unlike [Workstation mode](/fabric/workstation/workstation-server/) (loopback
...
- **Single- or multi-user.** [Tenancy](/fabric/choosing-a-mode/#tenancy) is orthogonal to the
...
The `sqlite` database driver (`FABRIC_SERVER_DATABASE_DRIVER=sqlite`) is what pins the Hub to a
...
[HA hosted](/fabric/hosted/ha/overview/) instead.

## How it differs from HA hosted

| | Single-node hosted | [HA hosted](/fabric/hosted/ha/overview/) |
|--|--|--|
...
- [Hub Setup](/fabric/hosted/single-node/hub-server/) — configure and run the Hub.
- [Deploy on a VM (GCE)](/fabric/hosted/single-node/hub-setup-gce/) — the starter-hub path.
- [Auth & Tenancy](/fabric/hosted/single-node/auth/) — single- vs multi-user access.
- [Connecting to a Hub](/fabric/hosted/user/hosted-user/) — the user-facing journey.
