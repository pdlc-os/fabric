...
description: Managing environment variables and secrets via the Fabric Hub.
---

Fabric's hosted architecture provides a centralized way to manage configuration and sensitive data across your team. Instead of sharing `.env` files or hardcoding credentials, you can use the Fabric Hub to store and inject environment variables and secrets into your agents.

## Variables vs. Secrets

Fabric distinguishes between regular environment variables and secure secrets:
...
Both variables and secrets can be scoped to different levels. Fabric resolves these hierarchically when an agent starts:
...
- **As Needed (Default)**: The variable or secret is only injected if it is explicitly requested in the agent's template (`fabric-agent.yaml`) or harness configuration. This is the recommended mode for most credentials to minimize the attack surface.
...
fabric hub env set --project --always LOG_LEVEL=debug

# Set a secret to be always injected for a user
fabric hub secret set --always MY_GLOBAL_TOKEN secret-value
```
...
Use the `fabric hub env` command suite to manage non-sensitive configuration.
...
fabric hub env set --broker=my-gpu-node CUDA_VISIBLE_DEVICES=0
```
...
fabric hub secret set --project DB_PASSWORD my-secure-password
```

**Interactive Secrets-Gather:**
If a template requires specific secrets (defined in `fabric-agent.yaml`), Fabric utilizes an interactive `secrets-gather` pipeline during agent creation. It will automatically prompt you to securely input any missing values and store them in the backend, ensuring sensitive credentials are never written to plain text configuration files.
...
3.  **Variable**: Added to a JSON file at `~/.fabric/secrets.json` for programmatic access by the harness.
...
fabric hub secret set --type file --target ~/.ssh/id_rsa SSH_KEY @~/.ssh/id_rsa
```

### Well-Known Secrets

Fabric recognizes certain secret names and uses them for built-in platform features. Using the correct name causes the broker to perform additional setup automatically.

| Secret Name | Type | Target Path | Effect |
|-------------|------|-------------|--------|
| `fabric-telemetry-gcp-credentials` | `file` | `~/.fabric/telemetry-gcp-credentials.json` | Sets `FABRIC_OTEL_GCP_CREDENTIALS`, auto-enables GCP-native telemetry export, and reads `project_id` from the file if `FABRIC_GCP_PROJECT_ID` is not set. |
...
Once set, every agent that starts will have the credential file mounted at `~/.fabric/telemetry-gcp-credentials.json` and GCP-native telemetry will be enabled automatically — no additional environment variable configuration required. See [Metrics & OpenTelemetry](/fabric/hosted/single-node/metrics/#4-gcp-credentials-for-agent-containers-non-adc-environments) for the full setup guide.
...
Fabric requires a secrets backend to store secret values. The recommended backend is **GCP Secret Manager**.
...
```bash
export FABRIC_SERVER_SECRETS_BACKEND=gcpsm
export FABRIC_SERVER_SECRETS_GCP_PROJECT_ID=my-gcp-project
export FABRIC_SERVER_SECRETS_GCP_CREDENTIALS=/path/to/service-account.json
```

When GCP Secret Manager is configured, Fabric uses a **hybrid storage** model:
...
For a detailed overview of the security architecture, see the [Security Architecture Reference](/fabric/reference/security/).
