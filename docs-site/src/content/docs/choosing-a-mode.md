...
description: How to pick the right Fabric run mode — Local, Workstation, Single-node hosted, or HA hosted — from the two dimensions that separate them.
---

Fabric runs the same agents and the same CLI whether you are working alone on a laptop or operating a durable, always-on platform for a team. What changes between those situations is **how much infrastructure sits behind the agents** — and that is what a *mode* captures.

This page introduces the four run modes and helps you pick the one that fits your use case. The definitions here follow the canonical [`GLOSSARY.md`](https://github.com/pdlc-os/fabric/blob/main/GLOSSARY.md); when in doubt, the root glossary wins.
...
The **availability tier** describes the durability of the **Hub** — Fabric's control plane (see [Core Concepts](/fabric/concepts/)). It is fixed by the Hub's database driver, set with `FABRIC_SERVER_DATABASE_DRIVER`:
...
|------|---------------|--------------------|---------|----------------|
| **Local** | None (CLI only) | Local machine; git-worktree isolation | Single-user | Agents launched directly via the `fabric` CLI, no server |
...
Run Fabric with **no server at all**. Agents are launched directly through the `fabric` CLI, state lives on your machine, and isolation between agents comes from git worktrees. There is no Hub, no web dashboard, and no admin role — just you and the CLI.

Choose Local when: *"I just want agents on my machine, no server."*

Start here with the [Installation guide](/fabric/getting-started/install/) and the [Tutorial](/fabric/getting-started/tutorial/).

### Workstation mode

Run a single-tenant Fabric server — the **combo server**, which bundles the Hub, a Runtime Broker, and the Web dashboard into one process — on your own machine over loopback. This gives you the hosted experience locally: a visual dashboard, remote-style dispatch, and project management, without deploying shared infrastructure. It is a local *server*, not the no-server CLI workflow.

Choose Workstation when: *"Give me the hosted experience locally."*

The fastest way in is the [Onboarding Wizard](/fabric/getting-started/onboarding/), which `fabric server start` opens automatically on first run. For the mechanics of the combo server, network bridges, and lifecycle, see [Workstation Server Mode](/fabric/workstation/workstation-server/).
...
Because it is networked, this tier forks into a **user** journey (connect, dispatch, collaborate) and a moderate **admin** journey (provision the node, configure auth and secrets). See [Hub Setup](/fabric/hosted/single-node/hub-server/).
...
This tier requires the fullest **user/admin** split: admins provision Postgres, object storage, load-balanced Hub replicas, proxy/hosted brokers, identity providers, and observability; users largely reuse the hosted-user journey. See [Hub Setup](/fabric/hosted/single-node/hub-server/) for the deployment tracks.

## How to choose

- **No server, just me** → **Local**. Nothing to run beyond the CLI.
- **The dashboard and hosted workflow, but only on my machine** → **Workstation**. One command (`fabric server start`) and the onboarding wizard.
...
- [Core Concepts](/fabric/concepts/) — Agent, Project, Hub, Runtime Broker, and the state model.
- [Glossary](/fabric/glossary/) — canonical definitions for every term used here.
- [Onboarding Wizard](/fabric/getting-started/onboarding/) — the guided setup for Workstation mode.
- [Workstation Server Mode](/fabric/workstation/workstation-server/) — running the combo server locally.
