...
```json
{
  ...
  "$id": "https://fabric.dev/schemas/settings/v1.json",
  "title": "Fabric Settings",
  "description": "Configuration for the Fabric agent orchestration platform (v1).",
  "type": "object",
  "properties": {
    "$schema": {
      "type": "string",
    ...
    "active_profile": {
      ...
      "x-env-var": "FABRIC_ACTIVE_PROFILE",
      "x-scope": "any",
      "x-since": "1"
    },
    "default_template": {
      ...
      "x-env-var": "FABRIC_DEFAULT_TEMPLATE",
      "x-scope": "any",
      "x-since": "1"
    },
    "server": {
      ...
      "properties": {
        ...
        "log_level": {
          ...
          "default": "info",
          "x-env-var": "FABRIC_SERVER_LOG_LEVEL",
          "x-since": "1"
        },
        "log_format": {
          "type": "string",
          "enum": ["text", "json"],
          "default": "text",
          "x-env-var": "FABRIC_SERVER_LOG_FORMAT",
          "x-since": "1"
        }
      },
    ...
    "hub": {
      ...
      "properties": {
        ...
        "local_only": {
          ...
          "x-env-var": "FABRIC_HUB_LOCAL_ONLY",
          "x-since": "1"
        }
      },
      "additionalProperties": false
    ...
    "cli": {
      ...
      "properties": {
        ...
        "interactive_disabled": {
          ...
          "x-env-var": "FABRIC_CLI_INTERACTIVE_DISABLED",
          "x-since": "1"
        }
      },
      "additionalProperties": false
  ...
  "$defs": {
    ...
    "serverHub": {
      "type": "object",
      "description": "Hub API server settings (for running fabric-server).",
      "properties": {
        "port": { "type": "integer", "default": 9810, "x-env-var": "FABRIC_SERVER_HUB_PORT" },
        "host": { "type": "string", "default": "0.0.0.0", "x-env-var": "FABRIC_SERVER_HUB_HOST" },
        "public_url": {
          "type": "string",
          "format": "uri",
          ...
          "x-env-var": "FABRIC_SERVER_HUB_ENDPOINT",
          "x-env-var-alias": "FABRIC_SERVER_HUB_PUBLIC_URL"
        },
        "read_timeout": { "type": "string", "default": "30s", "x-env-var": "FABRIC_SERVER_HUB_READTIMEOUT" },
        "write_timeout": { "type": "string", "default": "60s", "x-env-var": "FABRIC_SERVER_HUB_WRITETIMEOUT" },
        "cors": { "$ref": "#/$defs/corsConfig" },
        "admin_emails": {
          "type": "array",
          "items": { "type": "string", "format": "email" },
          "description": "Email addresses to auto-promote to admin role.",
          "x-env-var": "FABRIC_SERVER_HUB_ADMINEMAIL"
        }
      }
    },
    "serverBroker": {
      ...
      "properties": {
        "enabled": { "type": "boolean", "default": false, "x-env-var": "FABRIC_SERVER_BROKER_ENABLED" },
        "port": { "type": "integer", "default": 9800, "x-env-var": "FABRIC_SERVER_BROKER_PORT" },
        "host": { "type": "string", "default": "0.0.0.0", "x-env-var": "FABRIC_SERVER_BROKER_HOST" },
        "read_timeout": { "type": "string", "default": "30s", "x-env-var": "FABRIC_SERVER_BROKER_READTIMEOUT" },
        "write_timeout": { "type": "string", "default": "120s", "x-env-var": "FABRIC_SERVER_BROKER_WRITETIMEOUT" },
        "hub_endpoint": {
          "type": "string",
          "format": "uri",
          "description": "Hub API endpoint for this broker to report status to.",
          "x-env-var": "FABRIC_SERVER_BROKER_HUBENDPOINT"
        },
    ...
    "serverDatabase": {
      "type": "object",
      "properties": {
        "driver": { "type": "string", "enum": ["sqlite", "postgres"], "default": "sqlite", "x-env-var": "FABRIC_SERVER_DATABASE_DRIVER" },
        "url": { "type": "string", "x-env-var": "FABRIC_SERVER_DATABASE_URL" }
      }
    },
    "serverAuth": {
      "type": "object",
      ...
      "properties": {
        "dev_mode": { "type": "boolean", "default": false, "x-env-var": "FABRIC_SERVER_AUTH_DEVMODE" },
        "dev_token": {
          "type": "string",
          "description": "Dev auth token. Used by the server to accept dev requests and by the hub client to authenticate in dev mode. Also configurable via FABRIC_DEV_TOKEN env var or ~/.fabric/dev-token file.",
          "x-sensitive": true,
          "x-env-var": "FABRIC_SERVER_AUTH_DEVTOKEN"
        },
        "dev_token_file": { "type": "string", "x-env-var": "FABRIC_SERVER_AUTH_DEVTOKENFILE" },
        "authorized_domains": {
          "type": "array",
          "items": { "type": "string" },
          "x-env-var": "FABRIC_SERVER_AUTH_AUTHORIZEDDOMAINS"
        }
      }
    },
    "serverOAuth": {
    ...
    "serverStorage": {
      ...
      "properties": {
        "provider": { "type": "string", "enum": ["local", "gcs"], "default": "local", "x-env-var": "FABRIC_SERVER_STORAGE_PROVIDER" },
        "bucket": { "type": "string", "description": "GCS bucket name. All storage types are namespaced under path prefixes within this bucket.", "x-env-var": "FABRIC_SERVER_STORAGE_BUCKET" },
        "local_path": { "type": "string", "x-env-var": "FABRIC_SERVER_STORAGE_LOCALPATH" }
      }
    },
    "serverSecrets": {
      "type": "object",
      "properties": {
        "backend": { "type": "string", "enum": ["local", "gcpsm"], "default": "local", "x-env-var": "FABRIC_SERVER_SECRETS_BACKEND" },
        "gcp_project_id": { "type": "string", "x-env-var": "FABRIC_SERVER_SECRETS_GCPPROJECTID" },
        "gcp_credentials": { "type": "string", "x-env-var": "FABRIC_SERVER_SECRETS_GCPCREDENTIALS" }
      }
    }
  }
}
