...
- **Availability tier** — whether the Hub is a single instance on an embedded
  database or replicated on an external one. Fixed by `FABRIC_SERVER_DATABASE_DRIVER`
  (`sqlite` vs `postgres`).
...
| :--- | :--- | :--- | :--- | :--- |
| **Local** | None (CLI only) | Local machine; git-worktree isolation | Single-user | Agents via the `fabric` CLI, no server |
...
Installation, the **Onboarding Wizard** (`fabric server start` → `/onboarding`),
...
fabric-config-reference, security, server/web config.
...
are valid on disk (YAML preferred; `fabric init` writes YAML).
