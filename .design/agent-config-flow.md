...
Agent configuration in Fabric is assembled from multiple sources across two distinct
...
2. **Global settings** — `~/.fabric/settings.yaml`
3. **Grove settings** — `.fabric/settings.yaml`
4. **Environment variables** — `FABRIC_*` prefix (top-level settings keys only)
...
- `config.GetTemplateChain(templateName)` resolves the template by searching:
  1. Project templates (`.fabric/templates/<name>/`)
  2. Global templates (`~/.fabric/templates/<name>/`)
...
- For each template in the chain:
  ...
  - Load `fabric-agent.json` (or `.yaml`) from the template
  - Merge configs via `MergeFabricConfig()`
...
- A settings-sourced `FabricConfig` is built from the resolved harness's env, volumes,
  and auth type
- **The template config is merged OVER the settings config**, giving templates higher
  priority than settings for env/volumes:
  ```go
  finalFabricCfg = config.MergeFabricConfig(settingsCfg, finalFabricCfg)  // provision.go:247
  ```
...
- **`fabric-agent.json`** (in agent root) — the merged `FabricConfig` (harness, env,
...
- After harness provisioning, the `fabric-agent.json` is **reloaded** to pick up any
...
| Field | Source | File |
|-------|--------|------|
| `harness` | Template `fabric-agent.json` | `fabric-agent.json` |
| `env` | Template merged with settings harness/profile env | `fabric-agent.json` |
| `volumes` | Template + settings + workspace mount | `fabric-agent.json` |
| `image` | Template `fabric-agent.json` (if set there) | `fabric-agent.json` |
| `model` | Template `fabric-agent.json` | `fabric-agent.json` |
| `command_args` | Template `fabric-agent.json` | `fabric-agent.json` |
| `detached` | Template `fabric-agent.json` | `fabric-agent.json` |
| `kubernetes` | Template `fabric-agent.json` | `fabric-agent.json` |
| `gemini.auth_selectedType` | Settings harness/profile auth type | `fabric-agent.json` |
...
- Loads `fabric-agent.json` from the agent directory
- Reloads the template chain and re-merges: `template chain → agent's fabric-agent.json`
...
3. Template/agent config: `finalFabricCfg.Image` (overrides settings)
4. CLI `--image` flag (ultimate override)

**This is a start-time concern** — the image is NOT read from the persisted
`fabric-agent.json` env unless the template explicitly set it there.
...
1. `finalFabricCfg.Env` — from the persisted/re-merged agent config (template + settings
   harness env from creation time)
2. `opts.Env` (extra env from CLI) — includes `FABRIC_AGENT_NAME`, `FABRIC_TEMPLATE_NAME`,
   `FABRIC_BROKER_NAME`
...
Uses `finalFabricCfg.Volumes` from the persisted/re-merged config, with dedup logic to
...
| **`FABRIC_AGENT_NAME` etc.** | Injected at start | Runtime metadata |
| **Profile selection** | CLI `--profile` → saved → settings active | Allows changing profiles between runs |

---

## MergeFabricConfig Semantics (`templates.go:401-514`)
...
**At creation**: The template's `fabric-agent.json` may specify an `image`, and the
`--image` CLI flag is stored in `agent-info.json` (`provision.go:278-280`). But the
settings harness image is also merged into the `FabricConfig` env/volumes at creation.
...
the template chain and re-merges it with the agent's `fabric-agent.json`. This means if
...
finalization, the start path should load only from the persisted `fabric-agent.json`
...
into the persisted `fabric-agent.json`. But at start time (`run.go:105-111`), the settings
...
merge already baked the profile's env/volumes into `fabric-agent.json`.
...
There's no clear boundary where config is considered "done." The `fabric-agent.json`
...
- Finalize everything into `fabric-agent.json` at creation (except true runtime concerns)
...
At `provision.go:307-312`, after writing `fabric-agent.json`, the harness's `Provision()`
is called, which may modify files in the agent home. Then the config is **reloaded** from
disk. Currently only Claude's `Provision()` modifies `.claude.json` (not `fabric-agent.json`),
...
### Should be finalized at create time (snapshot into `fabric-agent.json`)
...
- **`FABRIC_AGENT_NAME`** and other injected metadata — runtime concerns
...
- **Pro**: Agents can be inspected (`fabric-agent.json`) to see exactly what they'll run with
...
`fabric-agent.json`. The benefit is that each agent's behavior is fully determined by its
