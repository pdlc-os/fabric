...
|--------|-------------|---------|
| [opencode](opencode/README.md) | [OpenCode](https://opencode.ai) AI coding assistant | `fabric harness-config install harnesses/opencode` |
| [codex](codex/README.md) | [Codex](https://github.com/openai/codex) OpenAI coding agent CLI | `fabric harness-config install harnesses/codex` |
| [antigravity](antigravity/README.md) | [Antigravity](https://github.com/ptone/fabric-antigravity) Gemini-based coding agent via OAuth | `fabric harness-config install harnesses/antigravity` |

Or install directly from GitHub (no local checkout needed):

```sh
fabric harness-config install github.com/pdlc-os/fabric/tree/main/harnesses/<name>
```
...
2. **Legacy config on `provisioner.type: builtin`** — the compiled-in Go
   ...
   ```sh
   fabric harness-config upgrade <name> --activate-script
   ```
   ...
   from the bundle:
   ```sh
   fabric harness-config install harnesses/<name>
   ```
...
<name>/
  ...
  fabric_harness.py     # Vendored copy of the shared library (generated)
  Dockerfile           # Image build (FROM fabric-base)
  cloudbuild.yaml      # Cloud Build configuration
...
Every provision.py should use the shared `fabric_harness` library:
...
AUTH = fabric_harness.AuthSpec(
    "<name>",
    [
        fabric_harness.env_method("api-key", any_of=["MY_API_KEY"]),
        # fabric_harness.file_method("auth-file", path="~/.myapp/creds.json"),
    ],
)
...
if __name__ == "__main__":
    fabric_harness.run("<name>", provision)
```
...
- `fabric_harness.project_instructions(...)` — managed-block instruction projection
- `fabric_harness.apply_mcp_translated(ctx, translate_fn, write_fn)` — MCP
  translation with warn-and-skip error handling
- `fabric_harness.apply_mcp_servers_simple(...)` — simple MCP merge into
  dotted-path config files

### Vendoring `fabric_harness.py`

The canonical source is `harnesses/fabric_harness.py`. Each bundle gets a
...
After editing `harnesses/fabric_harness.py`, regenerate vendored copies:
```sh
...
The `provisioner.lib` field in `config.yaml` controls how `fabric_harness.py`
is staged in the container:

- `vendored` (default) — the bundled `fabric_harness.py` is used as-is.
...
- `fabric_harness.py` declares `INTERFACE_VERSION` (currently `2`).
...
- provision.py should assert: `assert fabric_harness.INTERFACE_VERSION >= 2`
...
A `fabric harness-config list --available` command to discover installable
